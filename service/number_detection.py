...
class NumberDetection:
    ...
    def get_candidates(self, image, contours_dict, right_x, up_y, max_x, min_area):
    ...
    def calculate_plate_index(self, contour, plate_min_x, plate_min_y, plate_max_x, plate_max_y, min_area, image_area):
        x, y, w, h = cv2.boundingRect(contour)    
        area = w * h
        ratio = w / h
        min_width, min_height = image_area * 0.00000005, image_area * 0.0000002
    ...
    def parse_candidate(self, image, matched_result, img_thresh, min_area):
        height, width, channel = image.shape
        image_area = height * width
        plate_imgs = []
        plate_infos = []
        for _, matched_chars in enumerate(matched_result):
            self.plate_images(matched_chars, plate_imgs, plate_infos, img_thresh, width, height)
        ...
        for _, plate_img in enumerate(plate_imgs):
            ...
            for contour in contours:
                plate_min_x, plate_min_y, plate_max_x, plate_max_y = self.calculate_plate_index(contour, plate_min_x, plate_min_y, plate_max_x, plate_max_y,min_area, image_area)
    ...
    def find_chars_contour(self, d1, d2, matched_contours_idx):
    ...
    def find_chars(self, possible_contours, contour_list):
        ...
        for d1 in contour_list:
            ...
            for d2 in contour_list:
                if d1['idx'] == d2['idx']:
                    continue
                self.find_chars_contour(d1, d2, matched_contours_idx)
    ...
    def plate_images(self, matched_chars, plate_imgs, plate_infos, img_thresh, width, height):
    ...
    def filter_numbers(self, len_number, plate_imgs, channel, min_area):
        temp_plate = [] 
    ...
    def clustering(self, i, img_index, len_number, sorted_plate, max_diag):
        index = 0
    ...
    def ocr(self, plate_img, img_results, plate_chars):
    ...
    def detect_number(self, image, number, leftup, rightdown):
        ...
        img_thresh, contours_dict = self.preprocessing(image)
        ...
        matched_result = self.get_candidates(image, contours_dict, right_x, up_y, max_x, min_area)
        plate_imgs, img_results = self.parse_candidate(image, matched_result, img_thresh, min_area)
        len_number = len(number)
        temp_plate, possible_plate = self.filter_numbers(len_number, plate_imgs, channel, min_area)
        ...
        for i in range(len(x_sorted_plate)):    
            self.clustering(i, img_index, len_number, x_sorted_plate[i], max_diag)
        ...
        for plate_img in clopped_imgs:
            self.ocr(plate_img, img_results, plate_chars)
